...
public class PatchManager {
	...
	public PatchManager(Context context) {
		...
		Log.d(TAG, "PatchManager mPatchDir="+mPatchDir.getAbsolutePath());
	...
	private void initPatchs() {
		...
		Log.d(TAG, "initPatchs files.length="+files.length);
		for (File file : files) {
			Log.d(TAG, "initPatchs file name="+file.getName());
	...
	private Patch addPatch(File file) {
		...
		if (file.getName().endsWith(SUFFIX)) {
			try { 
				...
				Log.d(TAG, "addPatch 实例化Patch对象完成" );
	...
	private void loadPatch(Patch patch) {
		...
		List<String> classes;
		for (String patchName : patchNames) {
			Log.d(TAG, "loadPatch  patchName=" +patchName);
			if (mLoaders.containsKey("*")) { 
				cl = mContext.getClassLoader();//进入if获取类加载类
			} else { 
				cl = mLoaders.get(patchName);
			}
			if (cl != null) {
				classes = patch.getClasses(patchName);
				Log.d(TAG, "loadPatch  mAndFixManager.fix(patch.getFile(), cl, classes) size=" +classes.size() );
				mAndFixManager.fix(patch.getFile(), cl, classes);//执行这行会执行到dex2oat 直到所有方法修复完成
				Log.d(TAG, "loadPatch  mAndFixManager.fix(patch.getFile(), cl, classes); sucess" );
			}
		}
	}
